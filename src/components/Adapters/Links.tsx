...
export function TabLink(props: MUITabProps & Pick<LinkProps, 'to'>) {
  ...
  const renderLink = React.forwardRef<Link>((itemProps, ref) => <Link to={to} {...itemProps} ref={ref} />);

  const mergedCls = clsx(className, classes.tab);

  return (
    // @ts-ignore - ignoring missing `component` in TS definition.
    <Tab {...rest} value={to} component={renderLink} label={label} className={mergedCls} />
  );
}
